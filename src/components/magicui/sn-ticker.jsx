...
export function SNTicker({ className, direction = 'up', speed = 50 }) {
  return (
    <div className={cn("flex flex-col overflow-hidden select-none pointer-events-none opacity-[0.04] blur-[1px]", className)} aria-hidden="true">
      <motion.div
        animate={{
      ...
      >
        {TICKER_ITEMS.map((item, i) => (
          <span key={i} className="text-sm sm:text-base font-mono font-bold text-slate-900 tracking-widest uppercase">
            {item}
          </span>
        ))}
      </motion.div>
