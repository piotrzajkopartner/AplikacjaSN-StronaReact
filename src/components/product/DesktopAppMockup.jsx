...
function DesktopAppMockup({ className = '' }) {
  ...
  return (
    <section
      ...
      className={`relative overflow-hidden rounded-[1.35rem] border border-slate-200 bg-[#f6f7f9] [font-family:'Segoe_UI','Trebuchet_MS','Noto_Sans',sans-serif] shadow-[0_26px_70px_rgba(15,23,42,0.16)] ${className}`.trim()}
    >
      <header className="flex h-14 items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 sm:h-16 sm:px-5">
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-200 bg-white text-slate-500 shadow-sm hover:bg-slate-50 hover:text-slate-900 transition-colors"
          >
      ...
      {showQrModal && (
        <div
          className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4 backdrop-blur-xs"
          onClick={() => setShowQrModal(false)}
        >
          <div
            className="w-full max-w-xs rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-2xl animate-fade-in"
            onClick={(e) => e.stopPropagation()}
          >
            <h4 className="text-base font-bold text-slate-900">Otwórz demo na telefonie</h4>
            <div className="my-3 flex justify-center rounded-xl border border-slate-100 bg-slate-50 p-3">
              <img
                src={`https://api.qrserver.com/v1/create-qr-code/?size=180x180&data=${encodeURIComponent(demoUrl)}`}
                alt="Kod QR do strony demo"
                width={180}
                height={180}
            ...
            <p className="text-xs text-slate-500">
              Zeskanuj kod aparatem telefonu, aby otworzyć interaktywną wersję demo.
            </p>
            <button
              type="button"
              onClick={() => setShowQrModal(false)}
              className="mt-4 w-full rounded-lg bg-slate-100 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-200 transition-colors"
            >
              Zamknij
            </button>
          </div>
